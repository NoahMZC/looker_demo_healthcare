...
explore: condition_occurrence {
  ...
  join: location {
    relationship: many_to_one
    sql_on: ${person.location_id} = ${location.location_id} ;;
  }
...
join: cohorts_combined {
  view_label: "(3) Cohort Measurement"
  relationship: many_to_many
  sql_on: ${condition_occurrence.person_id} = ${cohorts_combined.person_id} ;;
}

join: cohort_sizes {
  relationship: many_to_one
  sql_on: ${cohorts_combined.cohort_name} = ${cohort_sizes.cohort_name} ;;
}
...
explore: person { hidden: yes }
explore: concept { hidden: yes }
explore: concept_ancestor { hidden: yes }
explore: location { hidden: yes }
